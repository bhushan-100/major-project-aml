...
def get_data(args, data_config):
    ...
    logging.info(f"Illicit ratio = {torch.sum(y).item()} / {y.shape[0]} = {torch.sum(y).item() / y.shape[0] * 100:.2f}%")
    ...
    if args.ports:
        ...
        te_data.add_ports()
        e = time.time()
        logging.info(f"Done: adding ports, took {int((e-s) // 60)}:{int((e-s)%60)}")
    if args.tds:
...
def get_data_cached(args, data_config):
    # Create a unique cache filename based on the input parameters
    cache_filename = f"cached_data_{args.data}_{args.ports}_{args.tds}_{args.model}_{'reverse_mp' if args.reverse_mp else 'no_reverse_mp'}.pkl"
    cache_path = os.path.join('.', cache_filename)
    ...
    # If no cache, run the original data processing (your existing code)
    tr_data, val_data, te_data, tr_inds, val_inds, te_inds = get_data(args, data_config)

    # Save the processed data to cache
    logging.info(f"Saving processed data to {cache_path}")
    with open(cache_path, 'wb') as f:
        pickle.dump((tr_data, val_data, te_data, tr_inds, val_inds, te_inds), f)
